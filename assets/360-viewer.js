...
(function() {
   ...
   'minis-box-container': {
    ...
    'combo-sativa-indica-horizontal': {
        name: 'Combo Sativa Indica Horizontal',
        nameHe: 'קופסא לשני מיכלי תפרחת אחד ליד השני',
        views: {
            closed: { frames: 6, label: 'סגור' },
            open: { frames: 5, label: 'פתוח' }
        ...
        Object.keys(PRODUCTS_360).forEach(productId => {
            ...
            allCards.forEach(card => {
             ...
             const productDescription = card.querySelector('.product-description');
                ...
                const searchText = product.searchInName ? productTitle : productDescription;
                if (searchText && searchText.textContent.includes(product.nameHe)) {
